...
{
	detail::abstract_transformer_backend_ptr new_rhs = sooty::common::clone_tree(rhs.backend);
	sooty::common::detail::append_success(new_rhs, detail::link_t(dummy().backend, detail::how_to_traverse::surface));
	detail::abstract_transformer_backend_ptr new_lhs = sooty::common::clone_tree(this->backend);
